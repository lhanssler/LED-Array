...
def flash(LED_array, LED_num, num_flashes, half_flash_length=0.5):
    ...
    Flashes an individual LED in on-off cycles. In one on-off cycle:
        Turns on individual LED by setting anode/cathode both to mode 1 (write) with high/low V, respectively.
...
def pwm(LED_array, LED_num, V_multiplier, duration=1.):
    '''
    Does PWM on an LED. Sets cathode to mode 1 (write) at low V, sets anode to mode 3 (PWM) at V_multiplier.
    Delays a specified amount of time, sets anode to low V, sets both cathode/anode to mode 0 (read) for default conditions.
    Parameters:
        LED_array (dictionary): Represents the LEDs and their connections to the Arduino board
            Keys are ints corresponding to LEDs; each LED is assigned an int key
            Values are 1D lists containing Arduino pin objects on the board in the form [anode_pin, cathode_pin]
        LED_num (int): Key of LED in LED_array which should do PWM
        V_apparent (float): Voltage multiplier of LED for PWM
            Must be between 0 (inclusive) and 1 (inclusive)
            Apparent voltage across LED would be 5.0 * V_multiplier
